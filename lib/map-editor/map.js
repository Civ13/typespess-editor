...
class Instance {
	...
	update_context(force = false) {
		...
		for(var key of ['icon', 'icon_state', 'dir', 'directional', 'layer', 'name', 'glide_size', 'screen_loc_x', 'screen_loc_y', 'overlays', 'x', 'y'])
			{client_instobj[key] = this.computed_vars[key];}
		if (!client_instobj.dir) {client_instobj.dir = 1;}
		if (!client_instobj.directional && this.computed_vars.components && this.computed_vars.components.Tangible)
			{client_instobj.directional = this.computed_vars.components.Tangible.directional;}
		else {client_instobj.directional = false;}
