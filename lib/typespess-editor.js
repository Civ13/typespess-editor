...
module.exports = {
	...
	reload_env() {
		...
		var envfile_path = path.join(root_dir,"config/ts-environment.cson");
		if(!fs.existsSync(envfile_path))
			throw new Error("Environment file does not exist! You must create a ts-environment.cson file in the config/ folder.");
		var env_paths = CSON.parse(fs.readFileSync(envfile_path, "utf8"));
		global.workspaceDir = root_dir;
		global.workspaceDir = global.workspaceDir.replace(`\\`,"/");
		global.workspaceDir += "/";
	...
	async create_thumbnail(instobj) {
		...
		var key = instobj.variant_leaf_path ? CSON.stringify(instobj) : "";
